...
##
## Purpose of script: define functions for plotting and saving figures
##
## Author: ANONYMOUS
##
...
# plots a histogram of culmen length for each species:
...
# Plots a qqplot in culmen length for each species:
plot_culmen_qqplot <- function(penguins_culmen){
  penguins_culmen %>% 
ggplot(aes(sample = culmen_length_mm)) +                        
  geom_qq() +                           
...
# plots final figure: histogram of culmen length in different Penguin Species, with 
# results of ANOVA and Tukey HSD.
...
plot_culmen_figure <- function(penguins_culmen){
  ...
  plot1 <-  ggplot(penguins_culmen2, aes (x = culmen_length_mm, fill = species)) +                  # use culmen length for x axis, colour bars by species
    ...
    xlab("Culmen length (mm)") +                                                                    # add axis labels, title and subtitle:
    ylab("Frequency") +
